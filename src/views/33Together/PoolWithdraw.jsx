      ...
      {!address ? (
        ...
        <Box className="withdrawal-container">
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
              ...
              <OutlinedInput
                ...
                startAdornment={
                  <InputAdornment position="start">
                    <div className="logo-holder">{srugImg}</div>
                  </InputAdornment>
                }
                labelWidth={0}
                endAdornment={
            ...
            >
              {exitFee > 0
                ? txnButtonText(pendingTransactions, "pool_withdraw", "Withdraw Early & pay " + exitFee + " sRUG")
                : txnButtonText(pendingTransactions, "pool_withdraw", "Withdraw sRUG")}
              {/* Withdraw sRUG */}
            </Button>
          </Box>
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography color="error" variant="body2">
                Withdrawing {quantity} sRUG reduces your odds of winning to 1 in {newOdds}&nbsp;
              </Typography>
            </Box>
          )}
          {exitFee > 0 && (
          ...
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had sRUG in during testing */}
