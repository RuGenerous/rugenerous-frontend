...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
  ...
  const wsrugBalance = useSelector<IReduxState, string>(state => {
  ...
  const hasAllowance = useCallback(() => srugAllowance > 0, [srugAllowance]);

  const trimmedSrugBalance = trim(Number(srugBalance), 6);
  const trimmedWsrugBalance = trim(Number(wsrugBalance), 6);

  const getBalance = () => (isWrap ? `${trimmedSrugBalance} sRUG` : `${trimmedWsrugBalance} wsRUG`);
      ...
      <Paper className="ohm-card ohm-popover wrap-token-poper">
        ...
        <div className="wrap-header-conteiner">
          ...
          <p className="wrap-header-balance">
            Balance: {isAppLoading ? <Skeleton width="80px" /> : <>{getBalance()}</>}
          </p>
        </div>
