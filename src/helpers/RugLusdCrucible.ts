...
import { rug_lusd, lusd } from "./AllBonds";
import { abi as RugLusdCrucibleABI } from "src/abi/RugLusdCrucible.json";
import { abi as UniswapIERC20ABI } from "src/abi/UniswapIERC20.json";
import { BigNumber, ethers } from "ethers";
import { addresses } from "src/constants";
import { getTokenPrice } from ".";
import { RugLusdCrucible, UniswapIERC20 } from "src/typechain";

export const calcAludelDetes = async (networkID: NetworkID, provider: StaticJsonRpcProvider) => {
  const crucibleAddress = addresses[networkID].CRUCIBLE_RUG_LUSD;
  ...
  let rugPrice = await getTokenPrice("rugenerous");
  let rugContractAddress = addresses[networkID].RUG_ADDRESS.toLowerCase();

  let lusdPrice = await getTokenPrice("liquity-usd");
  let lusdContractAddress = lusd.getAddressForReserve(networkID).toLowerCase();

  let rugLusdPrice = await rug_lusd.getBondReservePrice(networkID, provider);
  let rugLusdContractAddress = rug_lusd.getAddressForReserve(networkID).toLowerCase();
  ...
  usdValues[rugContractAddress] = rugPrice;
  usdValues[rugLusdContractAddress] = Number(rugLusdPrice.toString());
  ...
  // rewardToken is RUG for this Crucible
  ...
  let totalStakedTokensUsd = stakedRug * rugPrice + stakedLusd * lusdPrice;
